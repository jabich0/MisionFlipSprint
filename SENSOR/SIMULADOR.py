...
MQTT_TOPIC = os.getenv("MQTT_TOPIC", "greendelivery/trackers/telemetry")

mqtt_client = None
try:
    mqtt_client = mqtt.Client()
    mqtt_client.connect(MQTT_HOST, MQTT_PORT, 60)
    print(f"📡 MQTT conectado -> {MQTT_HOST}:{MQTT_PORT} | topic: {MQTT_TOPIC}")
...
def main():
    ...
    try:
        opcion = os.getenv("MODO_EJECUCION")

        if opcion:
            print(f"▶️  MODO_EJECUCION={opcion} (tomado de variables de entorno)")
        else:
            # Si no hay variable de entorno, pedirlo manualmente (modo interactivo)
